...
- 임계영역은 락(lock)을 얻은 단 하나의 쓰레드만 출입가능(객체 1개에 락 1개) (락 = 자물쇠)

> 1) synchronized를 이용한 동기화
- synchronized로 임계영역(lock이 걸리는 영역)을 설정하는 방법 2가지

## 17. wait()과 notify()
- 동기화를 하면 데이터가 보호가 되지만 한번의 한쓰레드만 임계영역에 들어 갈 수 있어서 효율이 떨어짐 
- 동기화의 효율을 높이기 위해 wait(), notify()를 사용
- Object클래스에 정의되어 있으며, 동기화 블록 내에서만 사용할 수 있다.
  1) wait() : 기다리기 : 객체의 lock을 풀고 쓰레드를 해당 객체의 waiting pool에 넣는다.
  2) notify() : 통보, 알려주기 : waiting pool에서 대기중인 쓰레드 중의 하나를 깨운다.
